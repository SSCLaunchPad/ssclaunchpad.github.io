...
3. If you click that, you'll see the role(s) you have available on that particular account; likely one called `PowerSysadmin`
4. Click on the role you'd like to use, and a new window/tab will open to that role in that account.
...
- The **Start** page is intolerant of you openning multiple accounts at the same time
  - Doing so will render inoperable any other AWS tabs you have open prior to doing so
  - Once inside the management console, open as many windows/tabs as you like, so long as you don't start again from the *Start* page
- If you need to create Roles, you're welcome to do so, however you will need to attach a Boundary Policy. 
  - You can see the full details about that [here]() _(need new link)_
...
4. Once authenticated select the account and role you'd like to use
...
5. Enter the *CLI default client region*, as `ca-central-1`
6. Enter the *CLI default output format*, as `json`
7. For the *CLI profile name*, you can leave the default (which is `<Role>-<AccountNumber>` by default), or specify one of your own
...
- **Environment Variable**  
  Linux: `export AWS_DEFAULT_PROFILE=<ProfileNameFromStep8Above>`  
  Windows CMD: `set AWS_DEFAULT_PROFILE=<ProfileNameFromStep8Above>`  
  PowerShell: `$env:AWS_DEFAULT_PROFILE = '<ProfileNameFromStep8Above>'`  
- **Command-line**  
